...
function map_field_key($key) {
    $mapping = [
        // Mapeos bidireccionales para compatibilidad
        ...
        'velocitat-maxima' => 'velocitat-maxima',
        'any-fabricacio' => 'any',
        'any' => 'any',
...
function is_glossary_field($field_name) {
    return in_array($field_name, [
        'color-tapisseria',
        'carrosseria-cotxe',
        'segment', // Añadido para soportar segment (mapea a carroseria-cotxe)
        'traccio',
        'roda-recanvi',
        'color-vehicle',
        'tipus-tapisseria',
...
function process_and_save_meta_fields($post_id, $params, $is_update = false) {
    ...
    // Aplicar mapeo de campos para compatibilidad
    $mapped_params = [];
    ...
    // Solo aplicar valores por defecto durante la creación, no durante la edición
    if (!$is_update) {
    ...
    // Solo aplicar valores por defecto numéricos durante la creación
    if (!$is_update) {
    ...
    // Guardar campos booleanos solo si están presentes en los parámetros
    $boolean_fields = [
    ...
    foreach ($boolean_fields as $field) {
        if (isset($params[$field])) {
            $value = process_boolean_value($params[$field]) ? 'true' : 'false';
            update_post_meta($post_id, $field, $value);
        }
    }

    // Manejo especial para anunci-destacat (debe ser 0 o 1 y guardarse como is-vip)
    if (isset($params['anunci-destacat'])) {
        $value = process_boolean_value($params['anunci-destacat']) ? 1 : 0;
        update_post_meta($post_id, 'is-vip', $value);
        Vehicle_Debug_Handler::log("Guardado anunci-destacat como is-vip con valor: $value");
    }
    // Guardar campos numéricos específicos solo si están presentes
    if (isset($params['portes-cotxe'])) {
        update_post_meta($post_id, 'portes-cotxe', $params['portes-cotxe']);
    }
    if (isset($params['temps-recarrega-total'])) {
        update_post_meta($post_id, 'temps-recarrega-total', $params['temps-recarrega-total']);
    }
    if (isset($params['temps-recarrega-fins-80'])) {
    ...
    // Manejo especial para cables-recarrega y connectors (campos de glosario array)
    if (isset($params['cables-recarrega']) && !empty($params['cables-recarrega'])) {
        $values = $params['cables-recarrega'];
        if (!is_array($values)) {
            $values = [$values];
        }
        update_post_meta($post_id, 'cables-recarrega', $values);
        Vehicle_Debug_Handler::log("Guardado cables-recarrega: " . print_r($values, true));
    }

    if (isset($params['connectors']) && !empty($params['connectors'])) {
        $values = $params['connectors'];
        if (!is_array($values)) {
            $values = [$values];
        }
        update_post_meta($post_id, 'connectors', $values);
    ...
    foreach ($meta_fields as $field => $type) {
        ...
        if ($field === 'frenada-regenerativa' ||
            ...
            $field === 'revisions-oficials' ||
            $field === 'impostos-deduibles' ||
            $field === 'vehicle-a-canvi' ||
            $field === 'portes-cotxe' ||
